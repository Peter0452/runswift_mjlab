"""FastSAC actor and distributional critic networks.

Ported from Holosoma's FastSAC (inspired by FastTD3, arxiv 2505.22642).
"""
...
class Actor(nn.Module):
  ...
  def _setup_network(self, input_dim: int) -> None:
  ...
  def explore(
    ...
    std = log_std.exp()
    # Sim NaNs can poison obs → actor mean/std; clamp so one env doesn't kill training.
    mean = torch.nan_to_num(mean, nan=0.0, posinf=0.0, neginf=0.0)
    std = torch.nan_to_num(std, nan=1.0, posinf=1.0, neginf=1.0).clamp_min(1e-6)
    dist = torch.distributions.Normal(mean, std)
    raw_action = dist.rsample()
